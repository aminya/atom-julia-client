...
export default {
  ...
  handle(...a) {
    return this.ipc.handle(...Array.from(a || []))
  },
  input(m) {
  ...
  import(...a) {
    return this.ipc.import(...Array.from(a || []))
  },
  ...
  attach(conn) {
    this.conn = conn
    if ((typeof this.conn.ready === "function" ? this.conn.ready() : undefined) !== false) {
      this.flush()
    }
  ...
  clargs() {
    const { precompiled, optimisationLevel, deprecationWarnings } = atom.config.get("julia-client.juliaOptions")
    let as = []
