...
export default IPC = class IPC {
  ...
  msg(type, ...args) {
    return this.writeMsg([type, ...Array.from(args)])
  }
  ...
  import(fs, rpc = true, mod = {}) {
    ...
    if (fs.rpc != null || fs.msg != null) {
      ...
      fs.forEach(f => {
        return (mod[f] = (...args) => {
          if (rpc) {
            return this.rpc(f, ...Array.from(args))
          } else {
            return this.msg(f, ...Array.from(args))
          }
        })
      })
    }
  ...
  isWorking() {
    return this.loading != null ? this.loading.isWorking() : undefined
  }
  onWorking(f) {
    return this.loading != null ? this.loading.onWorking(f) : undefined
  }
  onDone(f) {
    return this.loading != null ? this.loading.onDone(f) : undefined
  }
  onceDone(f) {
    return this.loading != null ? this.loading.onceDone(f) : undefined
  }
  ...
  readStream(s) {
    ...
    s.on(
      ...
      (cb = bufferLines(m => {
        if (m) {
          return this.input(JSON.parse(m))
        }
      }))
    )
