...
export default {
      ...
      return new Promise((resolve, reject) => {
        ...
        proc.on("message", function(x) {
          if (x === "ready") {
            ...
            return console.log("julia server:", x)
          }
        })
  ...
  getStreams(id) {
    return Promise.all(["stdin", "stdout", "stderr"].map(s => this.getStream(id, s)))
  },
  ...
  get(path, args) {
    return this.activate()
      ...
      .then((...args1) => {
        const array = args1[0],
          id = array[0],
          [stdin, stdout, stderr] = Array.from(array[1]),
          events = array[2]
        return {
          stdin(data) {
          ...
          onStdout(f) {
            return stdout.on("data", f)
          },
          onStderr(f) {
            return stderr.on("data", f)
          },
          flush(out, err) {
            return cycler.flush(events, out, err)
          },
  ...
  start(path, args) {
    return this.activate().then(() => this.server.start(path, args))
  },
  ...
  serverAPI() {
    return {
      setPS(enabled) {
        return (basic.wrapperEnabled = () => enabled)
      },

      get: (path, args) => {
        return cycler.get(path, args).then(p => {
          p.id = this.pid += 1
          this.ps[p.id] = p
          return p.id
        })
    ...
    return ["socket", "stdout", "stderr", "stdin"].forEach(stream => {
      return ipc.handle(stream, id => {
        ...
        return (proc.attached = Promise.resolve(source).then(source => {
          ...
          if (stream === "socket") {
            return this.mutualClose(source, sock)
          } else {
            return sock.on("end", () => proc.kill())
          }
        }))
      })
    })
