...
export default {
  ...
  key(path, args) {
    return hash([path, ...Array.from(args)].join(" ").trim())
  },
      ...
      if (this.cache(path, allArgs).length < this.cacheLength) {
        const p = this.provider()
          ...
          .then(proc => {
            ...
            proc.socket.then(() => this.start(path, allArgs)).catch(e => this.removeFromCache(path, allArgs, obj))
  ...
  repl(ipc) {
    return ipc.rpc("changemodule", { mod: "Main" })
  },
  ...
  warmup(obj) {
    ...
    return obj.proc.socket
      .then(sock => {
        ...
        ;[this.boot, this.repl].forEach(
          f =>
            (obj.init = obj.init.then(function() {
              if (obj.cached) {
                return f(ipc)
              }
            }))
        )
        obj.init = obj.init.catch(err => console.warn("julia warmup error:", err)).then(() => ipc.unreadStream())
      })
      .catch(function() {})
  },
  ...
  reset() {
    return (() => {
      ...
      for (let key in this.procs) {
        ...
        result.push(ps.map(p => p.proc.kill()))
      }
      return result
    })()
  }
