...
const { changeprompt, changemodule, fullpath } = client.import({
  msg: ["changeprompt", "changemodule", "resetprompt"],
  rpc: ["validatepath", "fullpath"]
})
...
export function activate(_ink) {
  ...
  subs.add(
    atom.config.observe("julia-client.uiOptions.layouts.console.defaultLocation", defaultLocation => {
...
function newRemoteTerminal() {
  const term = ink.InkTerminal.fromId(
    `terminal-remote-julia-${Math.floor(Math.random() * 10000000)}`,
    terminalOptions()
  )
  term.attachCustomKeyEventHandler(e => handleKeybinding(e, term))
  term.onDidOpenLink(hasKeyboardModifier)
...
function handleLink(event, uri) {
  ...
  if (client.isActive()) {
    ...
    if (urimatch) {
      ...
      const matchregex = isWindows ? /(([a-zA-Z]\:)?[^\:]+)(?:\:(\d+))?/ : /([^\:]+)(?:\:(\d+))?/
