...
export function activate(_ink) {
  ...
  const buttons = [
    ...
    {
      tooltip: "Debug: Step to Selected Line",
      command: "julia-debug:step-to-selected-line",
      svg: buttonSVGs["step-to-selection"]
    },
  ...
  subs.add(
    atom.config.observe("julia-client.uiOptions.layouts.debuggerPane.defaultLocation", defaultLocation => {
...
client.handle({
  ...
  working() {
    client.ipc.loading.working()
  },
  doneWorking() {
    client.ipc.loading.done()
  },
  getFileBreakpoints() {
...
export function finish(ev) {
  requireDebugging(ev, () => client.import("finish")())
}
export function nextline(ev) {
  requireDebugging(ev, () => client.import("nextline")())
}
export function stepexpr(ev) {
  requireDebugging(ev, () => client.import("stepexpr")())
}
export function stepin(ev) {
  requireDebugging(ev, () => client.import("stepin")())
}
export function stop(ev) {
  requireDebugging(ev, () => client.import("stop")())
}
export function continueForward(ev) {
  requireDebugging(ev, () => client.import("continue")())
}
...
export function togglebp(cond = false, ed = atom.workspace.getActiveTextEditor()) {
  if (!ed || !ed.getPath()) {
