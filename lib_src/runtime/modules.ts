...
export default {
  ...
  isValidItem(item) {
    return __guard__(atom.views.getView(item), x => x.matches(this.itemSelector))
  },
  ...
  updateForItem(item = atom.workspace.getActivePaneItem()) {
    ...
    if (!this.isValidItem(item)) {
      this.itemSubs.add(
        __guardMethod__(item, "onDidChangeGrammar", o => o.onDidChangeGrammar(() => this.updateForItem()))
      )
  ...
  getEditorModule(ed, bufferPosition = null) {
    ...
    return getmodule(data).catch(err => {
      return console.log(err)
    })
  },
  ...
  activateView() {
    ...
    for (let x of [this.mainView, this.dividerView, this.subView]) {
    ...
    atom.tooltips.add(this.dom, { title: () => `Currently working in module ${this.current()}` })
