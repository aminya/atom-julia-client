...
export default {
  ...
  _showError(r, lines) {
    ...
    const lights = this.ink.highlights.errorLines(
      (() => {
        const result = []
        for (let { file, line } of lines) {
          result.push({ file, line: line - 1 })
        }
        return result
      })()
    )
    ...
    return r.onDidDestroy(() => {
      if ((this.errorLines != null ? this.errorLines.r : undefined) === r) {
        return this.errorLines.lights.destroy()
      }
    })
  },
  ...
  eval({ move, cell } = {}) {
    ...
    return Promise.all(
      codeSelector.get(editor).map(({ range, line, text, selection }) => {
        if (move) {
          codeSelector.moveNext(editor, selection, range)
        }
        const [start] = Array.from(range[0]),
          [end] = Array.from(range[1])
        ...
        if (rtype === "console") {
          ...
          setTimeout(
            () =>
              r != null
                ? r
                : (r = new this.ink.Result(editor, [start, end], {
                    type: rtype,
                    scope: "julia",
                    goto: scrollToResult
                  })),
          ...
          return evaluate({ text, line: line + 1, mod, path: edpath, errorInRepl })
            ...
            .then(result => {
              ...
              if (r == null || r.isDestroyed) {
                r = new this.ink.Result(editor, [start, end], { type: rtype, scope: "julia", goto: scrollToResult })
              }
