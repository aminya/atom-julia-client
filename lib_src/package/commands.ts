...
export default {
  activate(juno) {
    ...
    for (let scope of atom.config.get("julia-client.juliaSyntaxScopes")) {
      this.subs.add(
        atom.commands.add(`atom-text-editor[data-grammar='${scope.replace(/\./g, " ")}']`, {
    ...
    this.subs.add(
      atom.commands.add('atom-text-editor[data-grammar="source julia"]', {
...
.ink-workspace`,
    ...
    return this.subs.add(
      atom.commands.add("atom-workspace", {
        "julia-client:open-external-REPL"() {
          return juno.connection.terminal.repl()
        },
        "julia-client:start-julia"() {
          return disrequireClient("boot Julia", () => boot())
        },
        "julia-client:start-remote-julia-process"() {
          return disrequireClient("boot a remote Julia process", () => juno.connection.bootRemote())
        },
        "julia-client:kill-julia"() {
          return juno.connection.client.kill()
        },
        "julia-client:interrupt-julia": () =>
          requireClient("interrupt Julia", () => juno.connection.client.interrupt()),
        "julia-client:disconnect-julia": () =>
          requireClient("disconnect Julia", () => juno.connection.client.disconnect()),
        ...
        "julia-client:connect-external-process"() {
          return disrequireClient(() => juno.connection.messages.connectExternal())
        },
        "julia-client:connect-terminal"() {
          return disrequireClient(() => juno.connection.terminal.connectedRepl())
        },
        ...
        "julia:open-package-in-new-window"() {
          return requireClient("get packages", () => juno.runtime.packages.openPackage())
        },
        "julia:open-package-as-project-folder"() {
          return requireClient("get packages", () => juno.runtime.packages.openPackage(false))
        },
        "julia:get-help"() {
        ...
        "julia-client:activate-environment-in-parent-folder"(ev) {
          return requireClient("activate an environment", () =>
