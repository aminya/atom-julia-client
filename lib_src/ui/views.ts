...
export default views = {
  ...
  tree({ head, children, expand }, opts) {
    return this.ink.tree.treeView(
      this.render(head, opts),
    ...
    return (view = this.ink.tree.treeView(this.render(head, opts), [], {
      onToggle: once(() => {
        ...
        return getlazy(id).then(children => {
          ...
          return children
            .map(x => this.render(this.tags.div([x]), opts))
            .forEach(x => {
    ...
    return this.render(
      child.type === "tree"
        ...
        : // children: child.children.map((x) => @tags.span "gutted", x)
          this.tags.span("gutted", [label, child]),
      opts
    )
  },
  ...
  views: {
    dom(...a) {
      return views.dom(...Array.from(a || []))
    },
    html(...a) {
      return views.html(...Array.from(a || []))
    },
    tree(...a) {
      return views.tree(...Array.from(a || []))
    },
    lazy(...a) {
      return views.lazy(...Array.from(a || []))
    },
    subtree(...a) {
      return views.subtree(...Array.from(a || []))
    },
    link(...a) {
      return views.link(...Array.from(a || []))
    },
    copy(...a) {
      return views.copy(...Array.from(a || []))
    },
    number(...a) {
      return views.number(...Array.from(a || []))
    },
    code(...a) {
      return views.code(...Array.from(a || []))
    },
    latex(...a) {
      return views.latex(...Array.from(a || []))
    }
  },
...
;["div", "span", "a", "strong", "table", "tr", "td", "webview"].forEach(
  tag => (views.tags[tag] = (attrs, contents) => views.tag(tag, attrs, contents))
)
