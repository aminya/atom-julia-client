 ...
 */
export function getWordAndRange(
  editor,
  options = {
  ...
  const bufferPosition = options.bufferPosition ? options.bufferPosition : editor.getLastCursor().getBufferPosition()
 ...
 * - `wordRegex` {RegExp}: A RegExp indicating what constitutes a “word” (default: `wordRegex`).
 */
export function getWordRangeAtBufferPosition(
  editor,
  bufferPosition,
  options = {
    wordRegex
  ...
  const range = ranges.find(range => range.end.column >= column && range.start.column <= column)
