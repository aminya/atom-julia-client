...
export default {
  ...
  getVersion(path = this.jlpath()) {
    return new Promise((resolve, reject) => {
      ...
      return (proc = child_process.exec(`\"${path}\" --version`, (err, stdout, stderr) => {
        ...
        if (res == null) {
          return reject("Couldn't resolve version.")
        }
  ...
  script(...s) {
    return this.packageDir("script", ...Array.from(s))
  },
  ...
  getPathFromTreeView(el) {
    ...
    if (!pathEl) {
      ...
      if (activeEl) {
        pathEl = activeEl.querySelector("[data-path]")
      }
    }
    if (pathEl) {
  ...
  getDirPathFromTreeView(el) {
    ...
    if (!dirEl) {
      ...
      const activeEl = el.querySelector(".tree-view .selected")
      if (activeEl) {
        dirEl = activeEl.closest(".directory")
      }
    }
    if (dirEl) {
